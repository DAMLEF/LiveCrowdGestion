...
class TextZone(Button):
    def __init__(self, x: float, y: float, width: int, height: int, name: str, style: TextStyle,
        ...
        self.valid_button = TextButton(x + width + 3, y, height, height, "Valid_" + name, BaseStyle(70, BLACK), "X",
    ...
    def draw(self, window: pygame.Surface):
        ...
        if self.display_continuity:
            render = self.style.render(self.entry[self.get_min_in_render():])
        else:
            render = self.style.render(self.entry[0: self.get_max_in_render()])
